...
type InvoiceAuditHistoryProps = {
  invoiceId?: string;
  invoiceStatus?: string;
};

const DUMMY_AUDIT_HISTORY: InvoiceAuditEntry[] = [
  {
    id: "audit-8",
    action: "EDITED",
    user: "Chinmaya Das",
...
function getActionIcon(action: InvoiceAuditAction) {
  switch (action) {
    case "CREATED":
      return Circle;
    case "EDITED":
...
function getActionStyle(action: InvoiceAuditAction) {
  switch (action) {
    case "CREATED":
...
export function InvoiceAuditHistory({ invoiceId, invoiceStatus }: InvoiceAuditHistoryProps) {
  void invoiceId;
  void invoiceStatus;
  return (
